var faultTreeViewXSL = `<?xml version="1.0"?>
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
        ...
        <li>
          <xsl:element name="a">
            <xsl:attribute name="href">
              javascript:
              topEventTransform('<xsl:value-of select="$XmlFileName" />','<xsl:value-of select="$ftName" />');
              cutSetsTransform('CutSets_<xsl:value-of select="FaultTree[./Name=$ftName]/@ID" />','<xsl:value-of select="$ftName" />', 'unavailability', 'ascending','100', '0', '0', '0');
              menuTransform('<xsl:value-of select="$XmlFileName" />','Cut Sets View: <xsl:value-of select="$ftName" />');
              ...
              <xsl:choose>
                  <xsl:when test="$ftName=''">
                    ...
                    <xsl:apply-templates select="FaultTree">
						<xsl:with-param name="parentVal" select="$auxVal"/>
					</xsl:apply-templates>
                ...
                <xsl:otherwise>
                  ...
                  <xsl:apply-templates select="FaultTree[./Name=$ftName]">
                    ...
                    foldersTree.iconSrc = ICONPATH + "faultTree.gif";
                    foldersTree.iconSrcClosed = ICONPATH + "faultTree.gif";
                    initializeDocument();
  ...
  <xsl:param name="parentVal"/>
    ...
    <xsl:apply-templates select="TopNode">
		<xsl:with-param name="parentVal" select="$parentVal"/>
	</xsl:apply-templates>
    
  </xsl:template>

  <xsl:template match="TopNode" name="TopNode">
    <xsl:param name="parentVal"/>
    <xsl:apply-templates/>
    ...
    <xsl:value-of select="$parentVal" />.addChild(aux<xsl:value-of select="$subAuxVal" />
